...
/* Add client to queue */
// if new client(user) doesnot already exist, add it to the queue
...
void send_message(char *s, int uid){
	int i;
	for(i=0;i<MAX_CLIENTS;i++){
		if(clients[i]){ //  if clients[i] exist
			if(clients[i]->uid != uid){
...
void send_message_client(char *s, int uid){
	int i;
	for(i=0;i<MAX_CLIENTS;i++){
		if(clients[i]){   //  send message to client having uid == uid
			if(clients[i]->uid == uid){
				write(clients[i]->connfd, s, strlen(s));
			}
		}
...
void send_active_clients(int connfd){
	...
	char s[64];
	for(i=0;i<MAX_CLIENTS;i++){
		if(clients[i]){ // if clients[i] exists, print that
...
//sprintf
	sprintf(buff_out, "<<JOINED CONVERSATION , HELLO USER: %s\r\n", cli->name);
	...
	while((rlen = read(cli->connfd, buff_in, sizeof(buff_in)-1)) > 0){
		...
		if(buff_in[0] == '\\'){
			...
			if(!strcmp(command, "\\QUIT")){
				break; // break from  the conversation
				...
				send_message_self("<<COMMAND NOT KNOWN\r\n", cli->connfd);
			}
		}else{
			/* Send message */
			sprintf(buff_out, "[User %s]: %s\r\n", cli->name, buff_in);
	...
	/* Close connection */
	close(cli->connfd);
	sprintf(buff_out, "<<LEAVE, BYE FROM User %s\r\n", cli->name);
	send_message_all(buff_out);
...
//	signal(SIGPIPE, SIG_IGN);
	...
	while(1){
		...
		if((cli_count+1) == MAX_CLIENTS){
			...
			printf("<<REJECT: NO MORE CLIENTS ALLWOED ");
